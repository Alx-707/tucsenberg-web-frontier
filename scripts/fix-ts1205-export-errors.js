    ...
    for (const matchInfo of matches) {
      ...
      if (fs.existsSync(resolvedPath)) {
        ...
        if (exports.length > 0) {
          ...
          // 简单的启发式分类（可以根据需要改进）
          exports.forEach(exp => {
            if (exp.includes('Type') || exp.includes('Interface') || 
                exp.includes('Config') || exp.includes('Options') ||
          ...
          // 如果没有分类，全部作为值导出
          if (typeExports.length === 0 && valueExports.length === 0 && exports.length > 0) {
          ...
          if (replacement) {
            ...
            modified = true;
            console.log(`  修复 ${filePath}: 替换 export * from '${modulePath}' -> ${exports.length}个具体导出`);
          }
        }
      }
    }
...
function fixExportStarInDirectory(dirPath) {
  ...
  function processDirectory(currentPath) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        // 跳过不需要处理的目录
        if (!['node_modules', '.next', '.git', 'dist', 'build', 'backups'].includes(item)) {
