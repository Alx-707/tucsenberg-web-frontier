...
**必须在ESLint配置中正确定义Vitest全局变量**，避免`no-undef`错误：

```javascript
// eslint.config.mjs - Vitest测试文件配置
{
  name: 'vitest-config',
  files: [
    '**/*.test.{js,jsx,ts,tsx}',     // 标准测试文件
    '**/__tests__/**/*.{js,jsx,ts,tsx}', // __tests__目录
    'tests/**/*.{js,jsx,ts,tsx}',    // 根级tests目录
    'src/test/**/*.{js,jsx,ts,tsx}', // 测试设置文件
    'src/testing/**/*.{js,jsx,ts,tsx}' // 测试工具文件
  ],
  languageOptions: {
    globals: {
      describe: 'readonly',
      ...
      beforeAll: 'readonly',
      afterAll: 'readonly',
      vi: 'readonly',        // Vitest模拟工具
      vitest: 'readonly',    // Vitest全局对象
    },
  },
  rules: {
    '@typescript-eslint/no-explicit-any': 'off', // 测试文件允许any类型
    'no-magic-numbers': 'off', // 测试文件允许魔法数字
    'max-lines-per-function': ['warn', 160], // 测试函数长度放宽
  },
}
```
...
**标准测试文件路径**：
- `**/*.test.{js,jsx,ts,tsx}` - 与源文件同目录的测试文件
- `**/__tests__/**/*.{js,jsx,ts,tsx}` - __tests__目录下的测试文件
- `tests/**/*.{js,jsx,ts,tsx}` - 根级tests目录（集成测试、端到端测试）
- `src/test/**/*.{js,jsx,ts,tsx}` - 测试设置和配置文件
- `src/testing/**/*.{js,jsx,ts,tsx}` - 测试工具和辅助函数
...
// ✅ 第三方库Mock时允许any
const mockLibrary = {
  someMethod: vi.fn() as any, // 第三方库类型复杂时
};
...
export const setupTestEnvironment = () => {
  ...
  // Mock environment variables - 使用vi.stubEnv而不是Object.defineProperty
  vi.stubEnv('NODE_ENV', 'test');
  vi.stubEnv('NEXT_PUBLIC_SITE_URL', 'https://tucsenberg.com');
  vi.stubEnv('NEXT_PUBLIC_VERCEL_URL', 'tucsenberg.vercel.app');

  // Mock localStorage and sessionStorage - 完整的类型安全Mock
