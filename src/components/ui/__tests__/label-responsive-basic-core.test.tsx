...
describe('Label Responsive - Core Basic Tests', () => {
  describe('基本响应式样式', () => {
    ...
    it('handles mobile touch targets', () => {
      ...
      expect(label).toHaveClass('min-h-[44px]', 'min-w-[44px]', 'flex', 'items-center');
    });
  });
  ...
  describe('核心文本响应式', () => {
    it('handles responsive line height', () => {
      render(
        <Label className='leading-tight md:leading-normal lg:leading-loose' data-testid='responsive-leading'>
          Responsive Line Height
        </Label>
      );

      const label = screen.getByTestId('responsive-leading');
      expect(label).toHaveClass('leading-tight', 'md:leading-normal', 'lg:leading-loose');
    });

    it('handles responsive text alignment', () => {
      render(
        <Label className='text-left md:text-center lg:text-right' data-testid='responsive-align'>
    ...
    it('handles responsive text transform', () => {
      render(
        <Label className='normal-case md:uppercase lg:lowercase' data-testid='responsive-transform-text'>
  ...
  describe('边缘情况', () => {
    ...
    it('handles mixed responsive and non-responsive classes', () => {
      render(
        <Label className='font-bold text-blue-500 md:text-red-500 lg:text-green-500' data-testid='mixed-classes'>
          Mixed Classes
        </Label>
      );

      const label = screen.getByTestId('mixed-classes');
      expect(label).toHaveClass('font-bold', 'text-blue-500', 'md:text-red-500', 'lg:text-green-500');
    });
  });
});
