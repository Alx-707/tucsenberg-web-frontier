...
test.describe('Navigation System', () => {
  ...
  test.describe('Desktop Navigation', () => {
    test('should display all main navigation links', async ({ page }) => {
      ...
      const expectedLinks = ['Home', 'About', 'Services', 'Products', 'Blog', 'Performance Diagnostics'];
    ...
    test('should navigate between pages and highlight active link', async ({ page }) => {
      const nav = page.getByRole('navigation', { name: 'Main navigation' });
    ...
    test('should support keyboard navigation', async ({ page }) => {
      ...
      let attempts = 0;
      while (attempts < 10) {
        const isInNav = await focusedElement.locator('..').locator('nav').count() > 0;
        if (isInNav) break;
  ...
  test.describe('Mobile Navigation', () => {
    ...
    test('should open and close mobile navigation sheet', async ({ page }) => {
      ...
      const closeButton = mobileNavSheet.getByRole('button', { name: /close/i });
    ...
    test('should navigate from mobile menu and auto-close', async ({ page }) => {
      const mobileMenuButton = page.getByRole('button', { name: 'Toggle mobile menu' });
      await mobileMenuButton.click();
    ...
    test('should support touch interactions', async ({ page }) => {
      ...
      const sheetContent = mobileNavSheet.locator('[data-testid="mobile-menu-content"]');
  ...
  test.describe('Route Handling', () => {
    test('should handle non-existent routes with 404 page', async ({ page }) => {
