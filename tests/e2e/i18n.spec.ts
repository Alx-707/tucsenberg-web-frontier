...
test.describe('Internationalization (i18n)', () => {
  ...
  test('should default to English locale and display correct lang attribute', async ({ page }) => {
  ...
  test.describe('Language Switching', () => {
    test('should switch from English to Chinese and update content', async ({ page }) => {
    ...
    test('should show loading indicator during language switch', async ({ page }) => {
      const languageToggleButton = page.getByTestId('language-toggle-button');
      await languageToggleButton.click();
    ...
    test('should preserve current page path during language switch', async ({ page }) => {
  ...
  test.describe('Theme Localization', () => {
    test('should display theme toggle in correct language', async ({ page }) => {
      ...
      await expect(page.getByRole('menuitem', { name: /明亮模式/i })).toBeVisible();
      await expect(page.getByRole('menuitem', { name: /暗黑模式/i })).toBeVisible();
      await expect(page.getByRole('menuitem', { name: /系统模式/i })).toBeVisible();
    });

    test('should maintain theme preference across language switches', async ({ page }) => {
  ...
  test.describe('Content Translation Validation', () => {
    ...
    test('should handle missing translations gracefully', async ({ page }) => {
      ...
      const nav = page.getByRole('navigation', { name: 'Main navigation' });
      const diagnosticsLink = nav.getByRole('link', { name: 'Performance Diagnostics' });
  ...
  test.describe('Accessibility and i18n', () => {
    ...
    test('should have proper lang attributes for screen readers', async ({ page }) => {
  ...
  test.describe('URL and SEO', () => {
    test('should generate correct URLs for different locales', async ({ page }) => {
      // Test English URLs
    ...
    test('should handle direct navigation to localized URLs', async ({ page }) => {
