...
- **直接文件编辑** - 直接编辑 MDX 文件，无需额外界面
- **Git 工作流集成** - 版本控制集成，内容变更触发自动部署
...
- **React 组件集成** - 在 Markdown 中使用 React 组件
- **Front Matter 支持** - 元数据和配置管理
- **语法高亮** - 代码块语法高亮支持
- **自定义组件** - 可扩展的组件系统
...
- **文件系统映射** - 基于目录结构的语言分离
- **语言标识字段** - Front Matter 中的 `locale` 字段
- **同步更新机制** - 确保双语内容一致性
- **类型安全验证** - TypeScript 接口验证内容结构
...
http://localhost:3000/dev-tools  # 开发工具页面（仅开发环境）
```

#### React Scan 性能监控工作流

```bash
# 1. 启动开发服务器（React Scan 自动启用）
pnpm dev

# 2. 查看性能监控
# - 页面左下角：蓝色 "🔍 React Scan Active" 指示器
# - 页面右下角：控制面板和快捷键说明
# - 组件高亮：红色=需要优化，绿色=已优化
# - 浏览器控制台：性能警告和渲染统计

# 3. 可选：禁用 React Scan
pnpm dev:no-scan  # 或设置 NEXT_PUBLIC_DISABLE_REACT_SCAN=true

# 4. 测试和演示
# 访问 /dev-tools 页面查看演示组件
```

#### 内容处理流程

- **文件解析** - 使用 gray-matter 解析 Front Matter
- **MDX 编译** - @next/mdx 编译 MDX 内容
...
- **react-scan 0.0.42** - React 组件性能监控和渲染分析 ✅ **新增**
  - 开发环境自动启用，生产环境强制禁用
  - 实时检测不必要的重新渲染
  - 可视化性能瓶颈识别
  - 跟随开发服务器启用/关闭
