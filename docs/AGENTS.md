...
- When adding or modifying tests for i18n-aware components, layouts, or routes, you **MUST** use the centralized test utilities from `@/test/utils` (for example `renderWithIntl`, `createMockTranslations`) and message fixtures from `src/test/constants/mock-messages.ts` instead of creating new per-file `test-utils` or inline `mockMessages` that duplicate them.
- Local, file-scoped i18n mocks are allowed **ONLY** for highly specific core-configuration or full message-structure tests (e.g. `tests/unit/i18n.test.ts`); such files **MUST** include a short comment explaining why centralized mocks are not reused.
...
- For any non-trivial test work involving mocks (especially i18n, layout, and API integration tests), you **SHOULD** consult `docs/testing/mock-usage-guide.md` first and align with its patterns.
